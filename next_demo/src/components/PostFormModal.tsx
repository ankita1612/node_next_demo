...
import { X, User, MessageSquare, Calendar, AlertCircle, Loader2, FileText } from 'lucide-react';
      ...
      >
        ...
        <div className="flex items-center justify-between pb-5 mb-6 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              {isEditing
                ? 'Update post details and save changes'
                : 'Fill in post author, comment, and date'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {errorMessage && (
          <div className="mb-6 p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-sm flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-rose-900">Submission Failed</p>
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-5">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">
              Comment / Post Content <span className="text-rose-500">*</span>
            </label>
            <div className="relative">
              ...
              <textarea
                rows={4}
                placeholder="Enter post comment or message..."
                {...register('comment')}
                className={`w-full pl-10 pr-4 py-2.5 bg-slate-50 border rounded-xl text-sm font-medium text-slate-900 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 transition-all ${
                  errors.comment
                    ? 'border-rose-300 focus:border-rose-500 focus:ring-rose-500/20'
